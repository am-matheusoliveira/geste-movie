                ...
                <div class="card">
                    ...
                    <div class="card-body">                    
                        ...
                        <div class="card-body d-flex justify-content-between">
                            ...
                            <a href="{{ url('/director') }}" class="btn btn-sm btn-primary btn-insert-or-update" data-bs-toggle="modal" data-bs-target="#staticBackdropInsertUpdate">Novo Registro</a>
                        ...
                        <div class="card p-2">                        
                            ...
                            @if (count($result_director) > 0)
                                <table class="table table-striped" id="principal-table">
                                    ...
                                    <tbody>
                                        @foreach ($result_director as $result_director)
                                            ...
                                            <div class="modal-body">               
                                                ...
                                                <div class="form-row">
                                                    <div class="form-group col-md-12">
                                                        ...
                                                        <input type="text" class="form-control mask_date" id="director_birth_date" name="director_birth_date" placeholder="dd/mm/yyyy" required maxlength="10">
                                                    </div>
                                                </div>
                                            </div>
                                        </form>        
...
@section('footer')    
    ...
    <script>
        $(document).ready(function(event){
            ...
            function delete_register(id_director){
            ...
            $(document).on('click', '.btn-insert-or-update', function(event){
                ...
                if(event.target.textContent === 'Novo Registro'){
                    ...
                    $('#director_full_name').val(rowTable.cells[1].innerText);
                    $('#director_birth_date').val(rowTable.cells[2].innerText);
                }
            });            
            ...
            $('.insert-or-update').click(function(event){                                
                ...
                if(event.target.textContent === 'Editar'){
                    ...
                    rowTable.cells[1].innerText = $('#director_full_name').val();                
                    rowTable.cells[2].innerText = $('#director_birth_date').val();                

                    // CHAMANDO A FUNÇÃO PARA ATUALIZAR O REGISTRO
                    // OBS: closest() PROCURA O ELEMENTO MAIS PROXIMO AO ATUAL
                    update_register($(rowTable).data('registro'), $('#director_full_name').val(), $('#director_birth_date').val());

                }else if(event.target.textContent === 'Salvar'){

                    // CHAMANDO A FUNÇÃO PARA INSERIR O REGISTRO
                    insert_register($('#director_full_name').val(), $('#director_birth_date').val());

                }                
            });

            // ENVIAR A ATUALIZAÇÃO DO REGISTRO PARA O SERVIDOR
            function update_register(id_director, director_full_name, director_birth_date){
                
                // ENVIANDO A REQUISIÇÃO HTTP                  
                $.ajax({
                    url: "{{ route('director_update') }}",
                    type: 'POST',
                    dataType: "json",
                    data: {
                        id_director: id_director,
            ...
            function insert_register(director_full_name, director_birth_date){

                // ENVIANDO A REQUISIÇÃO HTTP                  
                $.ajax({
                    url: "{{ route('director_insert') }}",
                    type: 'POST',
                    dataType: "json",
                    data: {
                        director_full_name: director_full_name,
                    ...
                    success: function(response) {
                        ...
                        $("#principal-table").find('tbody')
                            .append(
                                $('<tr data-registro="' + response.id_director + '">').append(
